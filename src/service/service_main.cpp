...
BOOL CALLBACK SecureStopPrompt()
{
    ...
    // Запускаем MessageBox на Secure Desktop активной сессии
    DWORD result = 0;
    WTSSendMessageW(
...
bool ProtectProcessFromAll(HANDLE hProcess)
{
    ...
    // Администраторы - ЗАПРЕТ на завершение
    ...
    // Все остальные - ЗАПРЕТ на завершение
...
void StopService(handle_t h)
{
    ...
    // Запрашиваем подтверждение на Secure Desktop
    if (SecureStopPrompt())
    {
...
void StartAppInSession(DWORD sessionId)
{
    ...
    {
        // Защита от ВСЕХ, включая администраторов
